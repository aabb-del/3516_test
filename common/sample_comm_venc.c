...
#include "expand.h"
VENC_STREAM_CALLBACK_FUNC_T gVencStreamCallback=NULL;
...
HI_S32 SAMPLE_COMM_VENC_SaveStreamToFifo(int fd, VENC_STREAM_S* pstStream)
{
    HI_S32 i;
...
#if FLAG_ENABLE_FIFO
    HI_S32 fifoFds[VENC_MAX_CHN_NUM];
    HI_CHAR aszFifoName[VENC_MAX_CHN_NUM][64];
#endif
    ...
    ******************************************/
    ...
    for (i = 0; i < s32ChnTotal; i++)
    {
        ...
        if(PT_JPEG != enPayLoadType[i])
        {
...
#if FLAG_ENABLE_FIFO
            snprintf(aszFifoName[i],32, "/tmp/stream_chn%d_fifo", i);
            
            if(access(aszFifoName[i],F_OK) < 0){
                fifoFds[i] = mkfifo(aszFifoName[i],S_IRWXU);
                if(fifoFds[i] == -1){
                    perror("mkfifo");
            ...
            fifoFds[i] = open(aszFifoName[i],O_WRONLY); 
            if(fifoFds[i] == -1){
