...
static xop::MediaSessionId session_id;
...
/**
 * @brief 
 * 		地址格式为 rtsp://ip:port/name
 * @param name 
 * @param port 
 * @param ip 传入NULL的话去获取本机ip，否侧为设置的
 */
void rtsp_server_init(char *name,int port,char* ip)
{
	static std::string m_ip = "0.0.0.0";
	std::string local_ip;
	if(ip == NULL)
	{
		local_ip = xop::NetInterface::GetLocalIPAddress();
	...
	std::string str_maohao = ":";
	std::string str_xiekang = "/";
	std::string str_port = std::to_string(port);


	rtsp_url = prefix + local_ip + str_maohao + str_port + str_xiekang + name;

	server = xop::RtspServer::Create(event_loop.get());
	if (!server->Start(m_ip, port)) {
...
void rtsp_server_push_frame_264(char *data,int size,int frame_type)
{
	...
	videoFrame.type = frame_type; // 建议确定帧类型。I帧(xop::VIDEO_FRAME_I) P帧(xop::VIDEO_FRAME_P)
	...
	server->PushFrame(session_id, xop::channel_0, videoFrame); //送到服务器进行转发, 接口线程安全
}



RtspServer::RtspServer(char *url,int port)
{
...
void RtspServer::push_frame_264(char *data,int size,int frame_type)
{
	//获取一帧 H264, 打包
	xop::AVFrame videoFrame = {0};
	videoFrame.type = frame_type; // 建议确定帧类型。I帧(xop::VIDEO_FRAME_I) P帧(xop::VIDEO_FRAME_P)
