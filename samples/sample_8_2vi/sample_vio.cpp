...
HI_S32 SAMPLE_VIO_TEST(HI_U32 u32VoIntfType)
{
...
#else
    /* 第二个相机单独测试 */
    stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.MipiDev         = ViDev+1;
    stViConfig.astViInfo[s32WorkSnsId].stSnsInfo.s32BusId        = 1;
    stViConfig.astViInfo[s32WorkSnsId].stDevInfo.ViDev           = ViDev+1;
    stViConfig.astViInfo[s32WorkSnsId].stDevInfo.enWDRMode       = enWDRMode;
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.enMastPipeMode = VI_OFFLINE_VPSS_OFFLINE;
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[0]       = (ViDev+1)==0 ? ViPipe : -1;  
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[1]       = (ViDev+1)==1 ? ViPipe : -1;
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[2]       = -1;
    stViConfig.astViInfo[s32WorkSnsId].stPipeInfo.aPipe[3]       = -1;
    stViConfig.astViInfo[s32WorkSnsId].stChnInfo.ViChn           = ViChn;
    stViConfig.astViInfo[s32WorkSnsId].stChnInfo.enPixFormat     = enPixFormat;
    ...
    {
        SAMPLE_PRT("vi bind vpss success. s32Ret: 0x%x !\n", s32Ret);
    }
#endif
    ...
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Vi bind Venc failed. s32Ret: 0x%x !n", s32Ret);
        goto EXIT3;
    }
#else
