...
HI_S32 SAMPLE_VIO_TEST(HI_U32 u32VoIntfType)
{
    ...
    // VI_VPSS_MODE_E     enMastPipeMode = VI_ONLINE_VPSS_ONLINE; // vi在线vpss在线的时候不能给vpss送图像
...
// #endif
    ...
    std::thread thread_vb_to_vpss([&](){
        ...
        while(1)
        {
            ...
            if(HI_MPI_VPSS_SendFrame(0, 0, pstFrameInfo, -1) != HI_SUCCESS)
            {
                break;
            };
            usleep(25000);
